...
def main():
    ...
    VM_Output, ACT_Output, ez_run, y_act, y_prd, abnormal_p1_ez_all_run_q1 = fwc_p1_vm.VM_Run(lamda_PLS=v_PLS, Z=Z_VM, M=M)

    # fdh_graph.plt_show1(Z_VM * M, y_act[:, 0:1], y_prd[:, 0:1])
    # fdh_graph.plt_show2(Z_VM, ez_run[:, 0:1], ez_run[:, 1:2], Noise=True)

    fdh_graph.plt_show1(Z_VM * M, y_act[:, 0:1], y_prd[:, 0:1], 'Process-1 Abnormal Case ('+ str(Z_VM) + ' Metrology Runs)', '1')
