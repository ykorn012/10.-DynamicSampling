...
# SEED2 = 999111111
# 1000   818 조합
SEED1 = 100000  #111999999 3 1000  100000
SEED2 = 210  #1490 818 99 999999999 129 965 949
# Process 변수와 출력 관련 system gain matrix
...
def main():
    ...
    reactive_p2_ez_all_run_q2 = np.loadtxt(
        'D:/10. 대학원/04. Source/09. VM_Source/04. VMOnly/output/reactive_p2_ez_all_run_q2.csv', delimiter=",", dtype=np.float32)
    after_normal_p2_ez_all_run_q2 = np.loadtxt(
        'D:/10. 대학원/04. Source/09. VM_Source/04. VMOnly/output/after_normal_p2_ez_all_run_q2.csv', delimiter=",",
        dtype=np.float32)
    fdh_graph = FDC_Graph()
    ...
    fdh_graph.plt_show1(Z_VM * M, p2_y_act[:, 1:2], p2_y_prd[:, 1:2], 'Process-2 UpStream Rule Case with Process-1 Dynamic Sampling', '2')
    fdh_graph.plt_show2_4(Z_VM * M, reactive_p2_ez_all_run_q2[:, 1:2], p2_ez_all_run_q2[:, 1:2],
                          'Process-2 UPStream Rule X with Process-1 Dynamic Sampling', '2', color1='bx-', color2='rx--')
